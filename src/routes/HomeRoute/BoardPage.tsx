      ...
      <div className="card-body p-2">
        <div className="d-flex flex-row flex-wrap mb-2">
          {card.labelIds.map(id =>
            <span className="badge badge-secondary badge-pill mr-1 mt-1">
              {id.slice(0, 4)}
            </span>
          )}
        </div>
...
function ListComponent(props: ListComponentProps) {
  ...
  return (
    <div className={`card shadow-sm ${props.className}`} style={{
      width: "300px",
      minWidth: "300px",
    }}>
      ...
      <div className="card-body overflow-auto pt-2 pl-2 pr-2 pb-0">
        {cards.map(card =>
  ...
  return (
    <React.Fragment>
      <div className="bg-light border-bottom">
        <div className="container d-flex flex-row align-items-center pt-2">
          <h6 className="m-0">
            <b>{localState.board.name}</b>
          </h6>
          <ul className="nav nav-tabs border-bottom-0 text-nowrap ml-3">
            <li
